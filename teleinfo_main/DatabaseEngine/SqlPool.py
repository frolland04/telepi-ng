...
class SqlPool:
    ...
    def updateRhInst(self, val, ts):
        ...
        self.ex.execute(
            """ UPDATE T_RH_INST set RH = {:d}, TS = '{:s}' """.format(round(val), str(ts)))
    ...
    def updateTempInst(self, val, ts):
        ...
        self.ex.execute(
            """ UPDATE T_TEMP_INST set TEMP = {:.1f}, TS = '{:s}' """.format(val, str(ts)))
    ...
    def updatePaInst(self, val, ts):
        ...
        self.ex.execute(
            """ UPDATE T_PA_INST set PA = {:.2f}, TS = '{:s}' """.format(val, str(ts)))
