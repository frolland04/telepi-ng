...
CREATE TABLE IF NOT EXISTS `T_TELEINFO_HISTO` (
  ...
  `PRESSION_ATMOS` float DEFAULT NULL,
...
DROP TRIGGER IF EXISTS AutoMinMaxInsertInst |

CREATE TRIGGER AutoMinMaxInsertInst AFTER INSERT

ON T_TELEINFO_INST FOR EACH ROW
...
BEGIN
   ...
   DELETE FROM T_TELEINFO_MINMAX WHERE TS < DATE_SUB( NOW(), INTERVAL 60 DAY ) ;
